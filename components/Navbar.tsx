      ...
      >
        <div className="mx-auto max-w-7xl px-1 sm:px-3">
          <div className="flex h-[3.75rem] items-center justify-between gap-1 md:h-[4.25rem] md:gap-2 md:px-2">
            ...
            <Link href="/chi-siamo" className={`${navBtnLight} min-w-0 flex-1`}>
              <svg className="mb-0.5 h-6 w-6 shrink-0 md:h-7 md:w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
            ...
            <button type="button" onClick={() => setShowReviewModal(true)} className={`${navBtnLight} min-w-0 flex-1`}>
              <svg className="mb-0.5 h-6 w-6 shrink-0 md:h-7 md:w-7" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
              </svg>
              <span className="truncate">{t('nav.reviews')}</span>
            </button>

            <Link href="/asporto" className={`${navBtnLight} min-w-0 flex-1`}>
              <svg className="mb-0.5 h-6 w-6 shrink-0 md:h-7 md:w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
              ...
              <span className="truncate">{t('nav.takeaway')}</span>
            </Link>

            {/* Solo desktop: lingua + tema nella barra */}
            <div className="hidden shrink-0 items-center gap-2 border-l border-zinc-200 pl-3 dark:border-zinc-600 md:flex">
              <LanguageSwitcher variant="navDesktop" />
              <ThemeToggle variant="navDesktop" />
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
          ...
          >
            <div className="flex items-center justify-between border-b border-zinc-200/80 px-5 py-4 dark:border-zinc-700/80">
              <h2 className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-white">{t('nav.leaveReview')}</h2>
              <button
                type="button"
            ...
            <div className="flex flex-1 flex-col items-center justify-center space-y-5 overflow-hidden p-6">
              <p className="text-center text-[15px] leading-relaxed text-zinc-600 dark:text-zinc-300">{t('nav.reviewPrompt')}</p>
              <a
